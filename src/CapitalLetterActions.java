...
public class CapitalLetterActions {
    public static void main(String[] args) {
        ...
        myActions.sendKeys("kadir").build().perform();
        myActions.keyUp(myWebElement,Keys.SHIFT).build().perform();

         */
        Action action = myActions.moveToElement(myWebElement).click().keyDown(myWebElement, Keys.SHIFT).sendKeys("kadir").keyUp(myWebElement, Keys.SHIFT).build();
        action.perform();
